...
> 状态：**已实施**（2026-09-13，见 `feat/ai-assistance` 分支；三个前提决策已与维护者对齐）
> 前置依赖：基于 `fix/code-review-2026-09-13`（鉴权与安全修复）实施，本文引用的中间件与路由形态以该分支为准。
>
> 实施说明与设计稿的两处偏差：
>
> 1. 会话 cookie 的 `Secure` 属性改为环境变量 `COOKIE_SECURE=true` 显式开启（本地 Docker/http 部署默认关闭，TLS 部署开启）；
> 2. 会话为无状态 HMAC 令牌：注销仅清除浏览器 cookie，令牌在 7 天有效期内仍可用；吊销钥匙即时失效所有相关会话。
