<template>
  <div class="min-h-screen bg-[#F0FDFA] flex flex-col text-[#134E4A] font-sans transition-colors duration-200">
    
    <!-- Top Navigation (Web) -->
    <header class="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-200 shadow-sm hidden md:block">
      <div class="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
        <!-- Logo -->
        <div class="flex items-center space-x-2 cursor-pointer group" @click="router.push({ name: 'events' })">
        ...
        <nav class="flex items-center space-x-1">
          <button 
            ...
            :class="currentRoute === item.key ? 'bg-[#0D9488]/10 text-[#0D9488] font-medium' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900 cursor-pointer'"
          >
    ...
    <!-- Mobile Top Header -->
    <header class="md:hidden sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-200 shadow-sm">
      <div class="px-4 h-14 flex items-center justify-between">
        <div class="flex items-center space-x-2">
          <BookOpen class="w-5 h-5 text-[#0D9488]" />
          <span class="text-base font-semibold text-[#134E4A]">{{ pageTitle }}</span>
        </div>
        <button @click="router.push({ name: 'search' })" class="p-2 text-gray-500 hover:text-[#0D9488] transition-colors cursor-pointer">
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 pb-safe z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      <div class="flex items-center justify-around h-14 px-2">
        <button 
          ...
          :class="currentRoute === item.key ? 'text-[#0D9488]' : 'text-gray-500 hover:text-gray-900'"
        >
          <component :is="item.icon" class="w-5 h-5" :class="{ 'fill-current': currentRoute === item.key }" />
...
const pageTitle = computed(() => {
  return (route?.meta?.title as string) || (appRouter?.currentRoute?.value?.meta?.title as string) || '岁月史书';
});
