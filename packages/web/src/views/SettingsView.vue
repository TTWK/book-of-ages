    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col">
        <div class="flex items-center justify-between mb-6">
          ...
          <button
            @click="loadLogs"
            class="p-1.5 text-gray-400 hover:text-[#0D9488] bg-gray-50 hover:bg-[#F0FDFA] rounded transition-colors cursor-pointer"
          >
        ...
        <div
          v-else
          class="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar relative border-l-2 border-gray-100 ml-2 pl-4"
        >
          <div v-for="log in logs" :key="log.id" class="relative">
            ...
            <div class="flex justify-between items-start">
              <div>
                <div class="flex items-center space-x-2">
                  <span
                    ...
                    :class="getActionTextColor(log.action)"
                    >{{ log.action }}</span
                  >
                  <span class="text-sm font-medium text-gray-700">{{ log.entity_type }}</span>
                </div>
                <p
                  class="text-xs text-gray-500 mt-1 font-mono bg-gray-50 px-1 rounded inline-block"
                >
              ...
              <div class="text-right">
                ...
                <p
                  v-if="log.api_key_id"
                  class="text-[10px] text-[#F97316] font-medium mt-0.5 border border-orange-200 px-1 rounded inline-block"
                >
                  API
                </p>
              </div>
      ...
      <div v-else class="p-4 bg-[#F0FDFA] border border-[#CCFBF1] rounded-xl text-center">
        ...
        <div class="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden">
          <input
            ...
            readonly
            class="flex-1 p-2.5 text-sm font-mono text-gray-700 outline-none bg-transparent"
          />
          <button
            @click="copyKey"
            class="px-4 py-2.5 bg-[#0D9488] hover:bg-[#14B8A6] text-white font-medium transition-colors"
          >
            复制
          </button>
        </div>
      ...
      <template #footer>
        <div class="flex justify-end space-x-3">
          ...
          <button
            v-if="createdKey"
            @click="closeKeyModal"
            class="px-6 py-2 text-white bg-[#F97316] hover:bg-[#FB923C] rounded-md transition-colors font-medium"
          >
            完成
          </button>
        </div>
